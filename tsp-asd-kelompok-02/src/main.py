...
 
                if not produk_kadaluarsa:
                    ...
                    for p in produk_kadaluarsa:
                        status = 'MENDESAK!' if p.masa_kadaluarsa_hari <= 3 else 'Perhatikan' if p.masa_kadaluarsa_hari <= 7 else 'Normal'
...
 
                ...
                if buffer.is_empty():
                    ...
                    for i, p in enumerate(buffer.get_all(), 1):
                        print(f'  {i}. {p.kode} | {p.nama} | Kadaluarsa: {p.masa_kadaluarsa_hari} hari')
 
            else:
